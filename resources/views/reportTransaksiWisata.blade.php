...
<body>
	...
	<table>
		<thead>
			<tr>
				...
				<th width="70%">
				<h5 align="center">Laporan Transaksi Wisata</h5>
	...
	<p>Periode : {{ date('d-m-Y',strtotime($start)) }} sampai {{ date('d-m-Y',strtotime($end)) }}</p>
	<table class='table table-bordered'>
		...
		<tbody>
				...
				<tr>
					...
					<td>{{$dt->atas_nama}}</td>
					<td>{{$dt->dataKatalogWisata->nama}}</td>
					<td>{{$dt->jumlah_orang}}</td>
					<td>{{$dt->jumlah_hari}}</td>
					<td>{{$dt->dataKatalogWisata->tujuanWilayah->tujuan}}</td>
				...
				<td>Rp. {{number_format($nominalTotal,0,',','.')}}</td>
				</tr>
			</tbody>
		<!--  -->
	</table>
